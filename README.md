...
A few concrete numbers from the full analysis, for readers who want the headline results before diving into the details:

* **Fastest tuning method:** the automated PID optimization achieves the lowest cost function value (**J = 0.61**) among the three tuning methods, vs. **1.15** for the Simulink PID Tuner and **5.20** for manual tuning — see [Automated PID Optimization](#automated-pid-optimization).
* **Stability margins:** the PI and PID controllers maintain healthy stability margins (phase margin ≈ 64° and ≈ 60° respectively), confirmed independently through both frequency-domain analysis and closed-loop pole locations — see [Frequency-Domain Analysis](#frequency-domain-analysis) and [Pole-Based Stability Analysis](#pole-based-stability-analysis).
* **Robustness verified:** the selected PID controller remains stable and keeps satisfactory performance under ±30% variation of the motor's armature resistance and rotor inertia — see [Robustness Analysis](#robustness-analysis).
* **Implementation cross-checked twice:** the closed-loop response was validated independently through both a pure MATLAB (Control System Toolbox) model and the Simulink implementation, with practically identical results — see [Cross-Validation: MATLAB vs. Simulink](#cross-validation-matlab-vs-simulink).
* **Fully automated pipeline:** every figure, table and result file in this repository is generated by MATLAB scripts rather than produced by hand — see [Generated Outputs](#generated-outputs).
